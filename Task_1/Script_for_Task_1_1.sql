...
-- таблица ссылается на 3 предыдущих по полю take_param_id, unit_id и plant_id
...
-- таблица ссылается на tag_data.tag по tag_id
CREATE TABLE tag_data.tag_dv 
	(
	tag_dv_id SERIAL NOT NULL,
	tag_dv_date TIMESTAMP NOT NULL,
	tag_dv_value NUMERIC(15) NOT NULL,
	
	tag_id SMALLINT NOT NULL,
	
	CONSTRAINT tag_dv_id_pkey PRIMARY KEY(tag_dv_id),
	CONSTRAINT tag_id_fk FOREIGN KEY (tag_id)
		REFERENCES tag_data.tag(tag_id)
		ON DELETE NO ACTION
		ON UPDATE NO ACTION
	
	-- CONSTRAINT tag_dv_value_chk CHECK (tag_dv_value > 0)
	);

-- удаление схемы каскадно
-- DROP SCHEMA tag_data CASCADE;
